...
def spectrogram_array_summary(
    audios_with_labels: List[Tuple[tf.Tensor, str]], step, name=""
):
    """
    ...
    assert n_audios == len(labels), "Number of labels must match number of audios"
    assert n_audios > 0

    # Batch spectrogram operations
    spectrograms = ddsp.spectral_ops.compute_logmag(tf_float32(audios), size=768)

    n_cols = 2
    n_rows = (n_audios + n_cols - 1) // n_cols

    # Manually specify exact size of fig for tensorboard
    fig, axs = plt.subplots(n_rows, n_cols, figsize=(8, 8), squeeze=False)

    for i, label in enumerate(labels):
        _plt_spec(spectrograms[i], axs[i // n_cols][i % n_cols], label)
...
def _get_reasonable_f0_min_max(f0_midi, max_spike=5.0, min_midi_value=5.0, pad=6.0):
    ...
    comb_masks = np.ma.array(f0_midi, mask=np.logical_or(diff_mask.mask, f0_mask.mask))
...
def pianoroll_summary(
    ...
    for i in range(batch_size):
        ...
        fig, ax, _ = pianoroll_plot_setup(figsize=(6.0, 4.0), xlim=[0, img.shape[1]])
